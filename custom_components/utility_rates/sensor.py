...
from .coordinator import UtilityRatesCoordinator

_LOGGER = logging.getLogger(__name__)

# Icons for each tier (used for dynamic icon on Rate Tier sensor)
TIER_ICONS = {
...
async def async_setup_entry(
    ...
    coordinator: UtilityRatesCoordinator = hass.data[DOMAIN][entry.entry_id]
    ...
    # Only add gas sensor if user configured gas rates
...
class UtilityRatesBaseSensor(CoordinatorEntity[UtilityRatesCoordinator], SensorEntity):
    ...
    def __init__(
        self,
        coordinator: UtilityRatesCoordinator,
        entry: ConfigEntry,
        sensor_type: str,
    ) -> None:
        ...
        Args:
            ...
            sensor_type: Identifier for this sensor type.
        """
...
class ElectricRateSensor(UtilityRatesBaseSensor):
    ...
    def __init__(
        self, coordinator: UtilityRatesCoordinator, entry: ConfigEntry
    ) -> None:
...
class RateTierSensor(UtilityRatesBaseSensor):
    """Sensor showing the current rate tier (peak, off_peak, etc.).

    Useful for automations like:
      - Turn off AC when tier = peak
      - Start EV charging when tier = super_off_peak
    """
...
class GasRateSensor(UtilityRatesBaseSensor):
    """Sensor showing the configured gas rate.

    This is a static value set by the user in config, since OpenEI
    doesn't provide gas rate data.
    """
