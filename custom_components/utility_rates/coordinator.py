...
class UtilityRatesCoordinator(DataUpdateCoordinator[dict[str, Any]]):
    ...
    def __init__(self, hass: HomeAssistant, entry: ConfigEntry) -> None:
        ...
        Args:
            ...
            entry: Config entry with API key, utility, and plan info.
        """
        super().__init__(
            hass,
            _LOGGER,
    ...
    async def _async_update_data(self) -> dict[str, Any]:
        ...
        Called every UPDATE_INTERVAL (1 minute). Fetches fresh rate data
        from OpenEI if API_FETCH_INTERVAL (24 hours) has passed.

        Returns:
            Dict with current rate info for sensors to consume.

        Raises:
            UpdateFailed: If rate calculation fails completely.
        """
        now = dt_util.now()
        ...
        if needs_fetch:
            try:
                ...
                if self._schedule is not None:
                    # We have cached data — log warning but keep going
                    _LOGGER.warning(
                    ...
                    # No cached data at all — this is a real failure
                    raise UpdateFailed(
        ...
        try:
            ...
            raise UpdateFailed(
                f"Error calculating current rate: {err}"
            ) from err

        # Build data dict for sensors
        result: dict[str, Any] = {
