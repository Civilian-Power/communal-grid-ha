...
class OpenEIClient:
            ...
            try:
                ...
                async with self._session.get(
                    ...
                    if response.status == 429:
                        ...
                        _LOGGER.warning(
                            "OpenEI rate limit hit, waiting %ds before retry", wait_time,
                        )
                ...
                if attempt < retries - 1:
                    wait_time = 2 ** (attempt + 1)
                    _LOGGER.warning(
                        "OpenEI request failed (%s), retrying in %ds", str(err), wait_time,
                    )
                    await asyncio.sleep(wait_time)
                else:
                    raise OpenEIConnectionError(
    ...
    async def validate_api_key(self) -> bool:
        ...
        try:
            await self._api_request({"limit": "1", "detail": "minimal"})
            return True
